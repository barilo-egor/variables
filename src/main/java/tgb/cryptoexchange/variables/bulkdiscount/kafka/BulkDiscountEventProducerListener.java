...
public class BulkDiscountEventProducerListener implements ProducerListener<String, List<BulkDiscountDTO>> {

    @Override
    public void onSuccess(ProducerRecord<String, List<BulkDiscountDTO>> producerRecord, RecordMetadata recordMetadata) {
        log.debug("Успешно отправлен ивент. Key={}, event={}.", producerRecord.key(), producerRecord.value());
    }

    @Override
    public void onError(ProducerRecord<String, List<BulkDiscountDTO>> producerRecord, RecordMetadata recordMetadata, Exception exception) {
